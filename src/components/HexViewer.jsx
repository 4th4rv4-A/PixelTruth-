  ...
  const renderRow = (rowIndex) => {
    ...
    for (let i = 0; i < BYTES_PER_ROW; i++) {
      if (i < rowBytes.length) {
        ...
        hexStr += `<span class="inline-block w-6 text-center ${isSelected ? 'bg-develop-500/30 text-develop-300' : ''}" data-offset="${offset + i}">${hex}</span> `;
        asciiStr += `<span class="${isSelected ? 'bg-develop-500/30 text-develop-300' : ''}" data-offset="${offset + i}">${(b >= 32 && b <= 126) ? String.fromCharCode(b) : '.'}</span>`;
      } else {
        hexStr += '<span class="inline-block w-6 text-center text-surface-700"></span> ';
        asciiStr += '<span class="text-surface-700"></span>';
      }
    }
    ...
    return (
      ...
      >
        ...
        <div className="w-[420px] text-surface-300 select-text cursor-text" dangerouslySetInnerHTML={{ __html: hexStr }} />
        <div className="flex-1 text-surface-400 whitespace-pre select-text cursor-text" dangerouslySetInnerHTML={{ __html: asciiStr }} />
      </div>
    );
  };
